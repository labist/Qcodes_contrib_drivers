...
# this driver is used for the fast IST DAC. It has to be perfectioned by a skilled programmer because I am not sure it
# is quite as fast or efficient as it could be...


class FastDuck(Instrument):
	def __init__(self, name, interface='', reset=False, numdacs=16, dac_step=10.0, dac_delay=.002, safe_version=True,
		...
		for i in range(int(self._numdacs / 4)):
			self.set_pol_dacrack(polarity[i], np.arange(1 + i * 4, 1 + (i + 1) * 4), get_all=False)
		...
		for i in range(1, numdacs + 1):
			self.add_parameter(
				...
				vals=vals.Numbers(self.pol_num[i - 1]-1, self.pol_num[i - 1] + self.Fullrange+1),
				step=dac_step,
