...
class FastDAC_VISA(VisaInstrument):

	def __init__(self, name, address, reset=False, numdacs=16, dac_step=10.0, 
				dac_delay=.002, safe_version=True,
	...
	def _send_and_read(self, message, bytestoread):
		...
		# self.ser.flushInput()
		self.write( message )
		reply = self.read(message_len=bytestoread)
		return reply
...
class FastDAC(Instrument):
	...
	def __init__(self, name, interface = 'COM5', reset=False, numdacs=16, dac_step=10.0, dac_delay=.002, safe_version=True,
				 use_locks=False, polarity=['BIP', 'BIP', 'BIP', 'BIP'],**kwargs):
		t0 = time.time()
